...
import { CreateUserParams, findOneUserByTypeAndUsername, RawUser, rawUserToUser } from "back-end/lib/db";
import {
  KEYCLOAK_URL,
 ...
 */
export const getLoggedAgent = async (realm:string, clientId: string, clientSecret:string,username:string, password:string):Promise<SuperTest<Test>> => {
  let tokenUrl = `${KEYCLOAK_URL}/auth/realms/${realm}/protocol/openid-connect/token`;
  ...
  const response = await externalAgent.post(tokenUrl)
                      .send(body)
                      .set("Cache-Control", "no-cache")
                      .set("Content-Type", "application/x-www-form-urlencoded")
    const authResponse = await agent.post(`${authService}?token=foobar`)
                              .send(response.body)
 ...
 */
export const getGovAgent = async (username:string = govUsername, password:string = govPassword):Promise<AgentWithCookie> => {
 ...
 * @returns he superagent instance
 */
export const getVendorAgent = async (username:string = vendor1Username, password:string = vendor1Password):Promise<AgentWithCookie> => {
...
export async function acceptTerms(username:string, agent:AgentWithCookie){
  ...
  return agent.put(`/api/users/${vendorId}`)
