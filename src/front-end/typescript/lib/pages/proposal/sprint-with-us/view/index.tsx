...
function makeInit<K extends Tab.TabId>(): PageInit<RouteParams, SharedState, State_<K>, Msg_<K>> {
  return isUserType({
    ...
    async success({ routePath, routeParams, shared, dispatch }) {
      ...
      // Get the proposal.
      const proposalResult = await api.proposals.swu.readOne(routeParams.opportunityId, routeParams.proposalId);
      // If the request failed, then show the "Not Found" page.
      if (!api.isValid(proposalResult)) { return fail(); }
      const proposal = proposalResult.value;
      const opportunityResult = await api.opportunities.swu.readOne(proposal.opportunity.id);
